...
def show_dashboard_silent():
    ...
    tab_lead, tab_all, tab_scan, tab_news = st.tabs(["🏆 LEADERS", "📋 ALL STOCKS", "🔍 SCANNER", "📰 NEWS BASE STOCKS"])
    ...
    with tab_news:
        if not df.empty:
            news_stocks = df.sort_values('VolRatio', ascending=False).head(40)
            g_cols = st.columns(7)
            col_idx = 0
            for i, (idx, row) in enumerate(news_stocks.iterrows()):
                news_data = get_detailed_news(row['Stock'] + ".NS")
                if news_data:
                    color = "#00c853" if row['Pts'] > 0 else "#ff5252"
                    tag_class = "tag-news-pos" if news_data['sentiment'] == "POSITIVE" else ("tag-news-neg" if news_data['sentiment'] == "NEGATIVE" else "tag-zone")
                    v_class = "vol-text-green" if row['VolRatio'] >= 2.0 else ""
                    with g_cols[col_idx % 7]:
                        st.markdown(f"""
                            <a href="https://www.tradingview.com/chart/?symbol=NSE:{row['Stock']}" target="_blank">
                                <div class="grid-box" style="border-left: 3px solid {color}; margin-bottom:10px;">
                                    <div style="display:flex; justify-content:space-between; align-items:center;">
                                        <span style="font-size:11px; font-weight:bold; color:#8a99a8;">{row['Stock']}</span>
                                        <span class="tag-box {tag_class}">NEWS {news_data['sentiment'][:3]}</span>
                                    </div>
                                    <div class="price-container" style="color:{color};">₹{row['LTP']}<br><span class="change-text" style="margin-left:0;">{row['Pct']}%</span></div>
                                    <div class="news-time">🕒 {news_data['time']}</div>
                                    <div style="display:flex; justify-content:space-between; border-top:1px solid #1c2a38; padding-top:4px; font-size:10px;">
                                        <span class="rsi-text-yellow">RSI:{row['RSI']}</span>
                                        <span class="{v_class}">x{row['VolRatio']} V</span>
                                    </div>
                                </div>
                            </a>""", unsafe_allow_html=True)
                        col_idx += 1
